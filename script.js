onload = function () {
    ...
    function createData(){
        ...
        // Prepares vis.js style nodes for my data
        ...
        for(let i=1;i<=V/2;){
            ...
            if(n1!==n2){
                ...
                // If works == 0, i can add bus as well as plane between vertices
                // If works == 1, i can only add plane between them
                if(works <= 1) {
                    if (works === 0 && i < V / 4) {
                        // Adding a bus
                        edges.push({
    ...
    genNew.onclick = function () {
        // Creating new data and display the data
        createData();
    ...
    function solveData() {
        ...
        // initializing min_dist to min distance via bus from src to dst
        let mn_dist = dist1[dst-1][0];

        // Seeing if plane should be used
        ...
        if(plane!==0){
            ...
            // Using spread operator to push elements of result of pushEdges to new_edges this is the most complicated and important func
